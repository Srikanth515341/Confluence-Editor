...
// Decisive test: R0002/R0003/R0005/R0006/R0007 (delayRelay.ts) and the two
// send-vs-receive runs above (both delayRelay.ts AND native routeWebSocket)
// all show the SAME "server-received-frame-count freezes permanently while
// the sender's own send() count keeps climbing and bufferedAmount stays 0"
// signature — on Firefox most often, but ALSO on WebKit once (native run,
// iter 7). This is now suspected to be a property of the delay-injection
// layer itself (both hand-rolled and Playwright-native), not the shipped
// product. This script runs the IDENTICAL instrumented scenario with ZERO
// delay-injection at all — browsers connect DIRECTLY to the real server,
// no relay, no routeWebSocket — to determine whether this freeze can
// happen with no injection layer in the path at all.
...
async function runOnce(iter, durationMs) {
  ...
  // DIRECT — no relay, no routeWebSocket. Browsers connect straight to the real server.
  ...
  // Also check: even without a text-level failure, did ANY session's receivedFrameCount ever
  // stop advancing while that same client's own sendCount kept climbing? This is the real
  // pass/fail criterion this investigation established — catch the freeze even if it never
  // grows large enough to also trip a content mismatch or the Case C canary in this run.
  const failed = !!crashLabel || !ok || !replayOk;

  console.log(
    `[SENDVSRECV-DIRECT] iter ${iter} order=${JSON.stringify(order)}:`,
  ...
  console.log(`  --- full sample timeline (always printed this run, not just on failure) ---`);
...
for (let i = 0; i < RUNS; i++) {
  ...
  try {
    ...
    console.log(`[SENDVSRECV-DIRECT] iter ${i}: HARD-CRASH ${e}`);
  }
  if (ok) passed++;
}
console.log(`\n${passed}/${RUNS} passed (SENDVSRECV-DIRECT diagnostic run)`);
